...
export const createOrder = async (req: Request, res: Response) => {
  try {
    ...
    const userId = (req as any).user?.id; // ดึง user id จาก auth middleware (ผูกไว้ที่ req.user)
    if (!userId) return res.status(401).json({ message: "Unauthorized" });

    // คำนวณราคาทั้งหมด + เตรียม payload สำหรับสร้าง orderItems
    // - ตรวจราคาเมนู/ท็อปปิงจาก DB
    // - สรุปยอดรวม (total) และ structure สำหรับ create
...
export const getOrderById = async (req: Request, res: Response) => {
  try {
    ...
    // ดึง order + child relations:
    // - orderItems
    //   - menuItem (เฉพาะฟิลด์ที่ต้องใช้)
    //   - orderItemToppings
    //       - topping (เฉพาะฟิลด์ที่ต้องใช้)
